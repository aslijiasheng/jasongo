...
class CI_Loader {
	...
	public function controller($controller, $name = "") {
		...
		foreach ($this->_ci_controller_paths as $con_path)
		{
			if ( ! file_exists($con_path.'controllers/'.$path.$controller.'.php'))
			{
				continue;
			}
			
			require_once($con_path.'controllers/'.$path.$controller.'.php');
		...
		show_error('Unable to locate the controller you have specified: '.$_ci_controllers);
